...
embedder.state_dict = torch.load('tmp_state_dict.pth')
embedder.eval()
...
def collate_fn(batch_tot):
    ...
    __, bn = embedder(input_, input_lens)
    
    bn = bn.view(bn.size(0), 1, bn.size(1))
    bn = tile(bn, 1, input_.size(1))

    input_ = torch.cat([input_, bn], dim=2)
...
for _rnn_type in settings['rnn_type']:
    for _rnn_hidden_size in settings['rnn_hidden_size']:
        ...
        exp_name = f'{type_}_embedding100_{now_}'
        ...
        torch.save(best_model.state_dict, f'saved/_tmp_10_sd_{now_}.pt')
        torch.save(best_model, f'saved/_tmp_10_fm_{now_}.pt')
...
torch.save(best_model.state_dict, f'saved/10_sd_{now_}.pt')
torch.save(best_model, f'saved/10_fm_{now_}.pt')